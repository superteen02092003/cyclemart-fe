...
const getOrderSyncKey = (bikeTitle) => `ORDER_STATUS_SYNC_FALLBACK_${getCleanSyncKey(bikeTitle)}`;
        ...
        .map((payment) => {
          ...
          return {
            ...
            status: resolveOrderStatus(
              payment.orderStatus,
              savedStatus,
              payment.status === 'SUCCESS' ? 'PAID_WAITING_DELIVERY' : 'PENDING_PAYMENT'
            ),
  ...
  const handleUpdateStatus = async (orderObj, newStatus) => {
    try {
      ...
      setOrders((prev) => prev.map((o) => (
        getCleanSyncKey(o.bikeTitle) === getCleanSyncKey(orderObj.bikeTitle)
          ? { ...o, status: newStatus }
          : o
      )));
