    ...
    <div className="bg-white rounded-sm border border-border-light shadow-card p-5 relative overflow-hidden transition-all hover:shadow-md">
       <div className="flex justify-between items-start border-b border-border-light pb-4 mb-4">
         <div>
           <div className="flex items-center gap-2 mb-1.5">
             ...
             <span className="text-xs text-content-secondary font-medium tracking-wide">
               Mã GD: {order.paymentOrderId || 'Đang cập nhật...'}
             </span>
             <span className="text-xs text-content-tertiary">|</span>
       ...
       <div className="flex gap-4">
         ...
         <div className="flex-1 min-w-0">
           ...
           <div className="mt-2 bg-surface-secondary/50 p-2.5 rounded-sm border border-border-light">
              <p className="text-xs text-content-secondary flex items-start gap-1.5">
                <span className="material-symbols-outlined text-[1.1rem] text-navy">location_on</span>
       ...
       <div className="mt-5 flex flex-wrap justify-end gap-3 pt-4 border-t border-border-light bg-gray-50/50 -mx-5 -mb-5 px-5 pb-5 rounded-b-sm">
          ...
          <Link to={`/bike/${order.bikeId}`}>
             <button className="py-2 px-4 border border-border-light text-content-secondary hover:bg-surface-secondary text-xs font-bold rounded-sm transition-colors bg-white shadow-sm">Xem chi tiết xe</button>
          </Link>
       </div>
    </div>
  )
...
export default function OrdersPage() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      let paymentData = [];
      try {
        const res = await api.get('/v1/payments/history?page=0&size=100');
        paymentData = res.data?.content || res.data || [];
      } catch (err) {
        console.error("Lỗi lấy đơn mua:", err);
      }

      const buyerOrders = paymentData
        .filter(p => !p.type?.includes('PRIORITY') && !p.type?.includes('INSPECTION')) 
        .map(payment => {
          ...
          return {
            ...
            status: payment.orderStatus || (payment.status === 'SUCCESS' ? 'PAID_WAITING_DELIVERY' : 'PENDING_PAYMENT'),
            counterpartName: payment.sellerName || 'Người bán',
            
            // 🔥 Ưu tiên lấy trường address từ DTO PaymentResponse
            deliveryAddress: payment.address || payment.deliveryAddress || 'Đang cập nhật địa chỉ...',
        ...
        sellerOrders = postData
          ...
          .map(post => {
            return {
              ...
              price: post.price,
              status: post.orderStatus || 'PAID_WAITING_DELIVERY',
              counterpartName: post.buyerName || 'Khách hàng',
              
              // Đọc từ BikePostResponse
              deliveryAddress: post.deliveryAddress || 'Đang tải địa chỉ giao hàng...',
  ...
  const handleUpdateStatus = async (orderObj, newStatus) => {
    if (!orderObj.paymentOrderId) {
      alert("Đơn hàng này chưa có mã hóa đơn hợp lệ từ máy chủ, không thể cập nhật.");
      return;
    }

    try {
      await api.put(`/v1/payments/${orderObj.paymentOrderId}/order-status?status=${newStatus}`);
      await fetchOrders(); // Reload lại API để lấy dữ liệu mới nhất
